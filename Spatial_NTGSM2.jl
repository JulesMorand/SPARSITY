...
@everywhere function calculate_damage_NT(ion, LET, integral, theta, Gyr, NT)
    ...
    if NT == false
        ...
        kappa_DSB = 0.5*9*calculate_kappa(ion,LET);
        lambda_DSB = kappa_DSB*10^-3;
    end
...
@time begin
    ...
    X = Array{Float64}(undef, 0, Nd);
    Y = Array{Float64}(undef, 0, Nd);
    N = Array{Float64}(undef, 0, Nd);
    M = Array{Float64}(undef, 0, Nd);
    for i in 1:Np
        #println(i)
        x,y= GenerateHit(R,Rk)
        track=Track(x,y,Rk)
        ...
        theta_ = [theta[1:end-1]./2 theta[2:end]./2]
        theta = minimum(theta_, dims = 2)
        X_, Y_ = calculate_damage_NT(ion, LET, integral, theta, Gyr, false);
        N_, M_ = calculate_damage_NT(ion, LET, integral, theta, Gyr, true);
