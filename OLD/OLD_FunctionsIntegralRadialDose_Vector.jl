...
function arc_intersection_angle_vector(r::Float64, b::Float64, r_nucleus::Float64)
    if b < r_nucleus
        if r <= r_nucleus - b
            return 2 * π, 0
...
##############
function integrate_weighted_radial_track_vector( rMin::Float64, rMax::Float64, b::Float64, r_nucleus::Float64, nSteps::Int64,LET::Float64)
    ...
    f2 = GetRadialLinearDose(rMin,LET) * rMin * arc_weight2
    r2 = rMin
    for i in 1:nSteps - 1
        ...
        f2 = GetRadialLinearDose(r2,LET) * r2 * arc_weight2
...
#####################################
function distribute_dose_vector(cell::Cell3D, track::Track,LET::Float64)
    x_track, y_track = track.x,track.y
    ####We put the track in the refertial of the cell
    x_track = (x_track - cell.x) #* 1e3  # mm -> um ??
    y_track = (y_track - cell.y) #* 1e3  # mm -> um
    b = sqrt(x_track^2 + y_track^2)

    rMax = min(Rk, b + cell.r_nucl)
    ...
    if b <= cell.r_nucl
        ...
        if b + track.Rk < cell.r_nucl
            r_intersection = track.Rk
        else
            r_intersection = cell.r_nucl - b
        end

        area1 = π * r_intersection^2
        integral1, theta11, area1, radius1 = integrate_weighted_radial_track_vector(0., r_intersection, b, cell.r_nucl, 1000,LET);
        dose = sum(integral1)
        # dose = track.getRadialIntegral(0.0, r_intersection) * area1

        if rMax > r_intersection
            integral2, theta21, area, radius2 = integrate_weighted_radial_track_vector(r_intersection, rMax, b, cell.r_nucl,1000,LET);
        ...
        if rMax == track.Rk
            if track.Rk > cell.r_nucl - b
               theta1 = acos((b/(2*rMax) + rMax/(2*b) - (cell.r_nucl^2) / (2 * b * rMax)))
               theta2 = acos((b/(2*cell.r_nucl) - (rMax*rMax)/(2*b*cell.r_nucl) + (cell.r_nucl) / (2 * b)))
               area3 = π * cell.r_nucl^2 - (theta1 * track.Rk^2 + theta2 * cell.r_nucl^2 - track.Rk * b * sin(theta1))
            else
                area3 = π * (cell.r_nucl^2 - r_intersection^2)
            end
        end

        #dose /= area1 + area2 + area3
        Gyr=dose/(area1+area2+ area3)
       # println("area1: ",area1," area2: ",area2," area3: ",area3,"\n1 sumareas:",area1+area2+area3," aera cell:",cell.r_nucl*cell.r_nucl*π, "\n",area1+area2+area3-cell.r_nucl*cell.r_nucl*π)

        #nucleus.totalNucleusDose += dose
       # push!(nucleus.doses, dose)
        #push!(nucleus.times, track.getTime())

    elseif b <= cell.r_nucl + track.Rk
        #nucleus.intersectionCount += 1
        rMin = b - cell.r_nucl
        integral, theta, area, radius = integrate_weighted_radial_track_vector(rMin, rMax, b, cell.r_nucl, 1000,LET)
        dose = sum(integral)
        area2=area
        if rMax == track.Rk
            theta1 = acos((b/(2*rMax) + rMax/(2*b) - (cell.r_nucl^2) / (2 * b * rMax)))
