...
function distribute_dose(cell::Cell3D, track::Track)
    ...
    if b <= cell.r_nucl
        ...
        area,integral,Gyr=integrate_weighted_radial_track(0., r_intersection, b, cell.r_nucl,1000);
        dose=integral
        # dose = track.getRadialIntegral(0.0, r_intersection) * area1

        if rMax > r_intersection
            area,integral,Gyr=integrate_weighted_radial_track(r_intersection, rMax, b, cell.r_nucl,1000);
    ...
    elseif b <= cell.r_nucl + track.Rk
        ...
        rMin = b - cell.r_nucl
        area,integral,Gyr = integrate_weighted_radial_track(rMin, rMax, b, cell.r_nucl,1000)
        dose = integral
        area2=area
        if rMax == track.Rk
            theta1 = acos((b/(2*rMax) + rMax/(2*b) - (cell.r_nucl^2) / (2 * b * rMax)))
